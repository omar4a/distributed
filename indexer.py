...
def wait_for_crawlers_done():
    ...
    logging.info("Waiting for crawl activity to cease dynamically...")
...
if zip_path:
    ...
    index_completion_queue.send_message(MessageBody=json.dumps(status_message), MessageGroupId="index_completion")
...
def search_query_listener():
    ...
    while True:
        ...
        if messages:
            ...
            search_results_queue.send_message(MessageBody=json.dumps(results), MessageGroupId="search_results")
            msg.delete()
        else:
            time.sleep(1)
...
def indexer_process():
    ...
    # Start the termination-monitoring thread
    threading.Thread(target=wait_for_crawlers_done, daemon=True).start()
